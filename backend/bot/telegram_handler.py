...
class TelegramBotManager:
    def __init__(self) -> None:
        ...
        # State Management & Heavy Lifters
        self.compose_states = {} 
        self.search_states = {}
    ...
    async def request_login(self, update: Update, user_id: int):
        """Generates Google OAuth Link connected exclusively to Render backend."""
        ...
        kb = [[InlineKeyboardButton("🔗 Connect Google Account", url=login_url)]]
        text = "⚠️ *Google Workspace Required*\nPlease link your Gmail account securely to activate Agentic AI capabilities."
    ...
    async def start_command(self, update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
        ...
        if await self.db.is_blocked("telegram", str(user.id)):
            await update.message.reply_text("⛔ *Access Denied*: Your Telegram ID has been blocklisted by administrators.", parse_mode="Markdown")
            return
        ...
        if not db_user.get("is_verified"):
            await update.message.reply_text("⏳ *Pending Authorization*\nYour request has been forwarded to administrators. You will be notified upon approval.", parse_mode="Markdown")
            return
        ...
        welcome_text = (
            f"✅ *Welcome, {user.first_name}! Access Granted.*\n\n"
            "I am your Enterprise Email Assistant powered by Gemini 2.0 Agentic AI.\n"
            "I automatically learn your contacts, generate smart summaries, and manage Gmail securely.\n\n"
            "Click below to securely connect your inbox:"
        )
    ...
    async def settings_command(self, update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
        ...
        await update.message.reply_text("⚙️ *System Preferences*\nConfigure your assistant interactions:", parse_mode="Markdown", reply_markup=InlineKeyboardMarkup(kb))
    ...
    async def handle_voice(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        if not db_user or not db_user.get("auth_token"):
            return await self.request_login(update, user_id)
        ...
        try:
            ...
            # Pass directly into AI logic
            response_text = await self.ai_engine.agent_chat(transcript, user_id)
            await self._deliver_ai_response(msg, response_text, user_id, transcript)
    ...
    async def handle_text(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        if user_id in self.compose_states:
            return await self.process_manual_compose(update, user_id, text)
        if user_id in self.search_states:
            return await self.process_manual_search(update, user_id, text)

        # Core Agentic AI Pipeline
        await context.bot.send_chat_action(chat_id=update.effective_chat.id, action='typing')
        msg = await update.message.reply_text("✨ *AI Engine Thinking...*", parse_mode="Markdown")
        
        response_text = await self.ai_engine.agent_chat(text, user_id)
        await self._deliver_ai_response(msg, response_text, user_id)

    async def _deliver_ai_response(self, msg_obj, response_text: str, user_id: int, transcript: str = None):
        """Intelligently delivers text or voice depending on DB preferences and fallbacks."""
        prefs = await self.db.get_user_preferences(user_id) or {}
        voice_pref = prefs.get("voice_preference", "text")

        # Basic Keyboard
        kb = [[InlineKeyboardButton("⚙️ Menu", callback_data="menu_main")]]

        if voice_pref in ["voice", "both"]:
            try:
                # Use the powerful fallback synthesis (Google -> EdgeTTS)
    ...
    async def handle_button_actions(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        if data == "menu_main":
            ...
            kb = [
                [InlineKeyboardButton("📥 Inbox", callback_data="manual_read_0"), InlineKeyboardButton("✍️ Compose", callback_data="menu_compose")],
                [InlineKeyboardButton("🔍 Search DB", callback_data="menu_search_prompt")],
                [InlineKeyboardButton("⚙️ Settings", callback_data="menu_settings")]
            ]
            await query.edit_message_text("🎛️ *Main Dashboard*\nSelect an action below or just speak to the AI natively.", parse_mode="Markdown", reply_markup=InlineKeyboardMarkup(kb))
            return
        ...
        if data.startswith("sum_"):
            ...
            await query.edit_message_text("✨ *Generating Summary...*", parse_mode="Markdown")
            full_text = await self.gmail.read_full_email(user_id, msg_id)
            summary = await self.ai_engine.agent_chat(f"Summarize this email strictly: {full_text[:3000]}", user_id)
            await query.edit_message_text(f"📑 *AI Summary:*\n\n{summary}", parse_mode="Markdown", reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("🔙 Back", callback_data="menu_main")]]))
            return

        if data.startswith("read_"):
            msg_id = data.split("_")[1]
            full_text = await self.gmail.read_full_email(user_id, msg_id)
            await query.edit_message_text(f"📖 *Email Content:*\n\n{full_text[:3500]}", parse_mode="Markdown", reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("🔙 Dashboard", callback_data="menu_main")]]))
            return
        ...
        if data == "clear_att":
            ...
            await query.edit_message_text("🧹 *Cache Cleared*\nAll buffered attachments have been removed.", parse_mode="Markdown", reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("🔙 Dashboard", callback_data="menu_main")]]))
            return

        if data == "menu_compose":
            self.compose_states[user_id] = {"step": "to", "data": {}}
            await query.edit_message_text("✍️ *Manual Compose*\nEnter the recipient's email address:", parse_mode="Markdown", reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("❌ Cancel", callback_data="menu_main")]]))
            return

    # --- MANUAL STATE PROCESSORS (Fallback when AI is off) ---
    async def process_manual_compose(self, update: Update, user_id: int, text: str):
        ...
        if state["step"] == "to":
            ...
            # Auto-Send
            await update.message.reply_text("🚀 Transmitting Email...")
    ...
    async def process_manual_search(self, update: Update, user_id: int, text: str):
        self.search_states.pop(user_id, None)
        await update.message.reply_text(f"🔍 Searching DB for `{text}`...")
        # Use AI to build query
        query = await self.ai_engine.get_search_query(text)
        emails = await self.gmail.get_emails(user_id, query=query, max_results=5)
        
        if not emails:
            return await update.message.reply_text("📭 No results found.", reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("🔙 Dashboard", callback_data="menu_main")]]))
        ...
        kb.append([InlineKeyboardButton("🔙 Dashboard", callback_data="menu_main")])
